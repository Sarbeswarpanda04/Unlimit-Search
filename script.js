...
function renderRecentSearches() {
	...
	const items = history
		.map(
			(query) =>
				`<a class="recent-item" href="results.html?q=${encodeURIComponent(query)}">${escapeHtml(query)}</a>`
		)
		.join("");
...
function renderSuggestions(input) {
	...
	container.innerHTML = suggestions
		.map(
			(query) =>
				`<button type="button" class="suggestion-item" data-query="${encodeURIComponent(query)}">${escapeHtml(query)}</button>`
		)
		.join("");
